...
**Ultima actualizacion:** 2026-06-07 (Claude Code — cierre Plan 01-11 / Wave 7 cerrada; Sentry runtime init 3 runtimes + 4 CI lint gates extended scope final + UI-SPEC §8.4 stability contract enforced; 1 item nuevo `[GAP-NPM-AUDIT-PHASE7]` P2 — 13 vulns transitivas del install Sentry SDK).
...
| `[BUG-PII-STORAGE-PLAN-07]` | Schema actual del PII `user.{name,date_of_birth}_ciphertext bytea + _dek_ciphertext bytea` (Plan 01-04 + 01-07) NO permite reconstruir un `EncryptedField` completo: `decryptPII` necesita ademas IV, GCM auth tag, key id (kid), y version (v) que `encryptPII` produce. Consecuencia descubierta en Plan 01-10 Task 1: `GET /me/data` degrada `date_of_birth` y `name` a `null` con `logger.warn` porque no puede recomponer los blobs. Cumple COMPL-05 estructuralmente pero el campo viene vacio. **Fix**: anadir columnas `{name,date_of_birth}_iv bytea + _tag bytea + _kid text + _v smallint` via migration 009b (o migrar el conjunto a una columna `_encrypted jsonb` con shape `{ciphertext, dek_ciphertext, iv, tag, kid, v}` para flexibilidad). Migration con `UPDATE` que repuebla iv/tag desde un re-encrypt — ningun usuario produccion aun, asi que safe wipe+repopulate o forzar re-prompt de DOB en Phase 2 magic-link OTP. | Fundacion / Fase 1 (Plan 01-12 fix opcional) | CC | Descubierto 2026-06-07 ejecutando Plan 01-10 Task 1 (decryptPII signature requires structured EncryptedField). No bloquea Phase 1 ship porque /me/data return shape es valido y `audit_log` registra `user_data_export`; bloquea Phase 2 cuando UX necesite ver DOB real. |
| `[GAP-DELETE-ATOMIC-TX]` | Plan 01-10 Task 1 implemento `DELETE /me/data` en dos fases NO atomicas: (1) Postgres transaction via Drizzle `db.transaction` (anonimiza audit/usage/distress via `anonymize_user_audit` RPC + DELETE user → CASCADE FK borra 7 tablas BORRAR D1.5), (2) FUERA de la transaction: `supabase.auth.admin.deleteUser(user.id)` contra auth schema separado de Supabase. Si (2) falla tras (1) commit, queda **orphan row en `auth.users`** sin contraparte `public.user`. Riesgo aceptado documentado en SUMMARY 01-10. **Fix**: emitir `supabase/migrations/010_delete_user_account.sql` con funcion SECURITY DEFINER `public.delete_user_account(target_user_id)` que reciba el user_id, ejecute anonimizacion + DELETE FROM public.user + `auth.admin_delete_user(target_user_id)` (Supabase Auth GoTrue >=v2.x expone admin function) en UNA SOLA transaction Postgres. Llamar via `.rpc('delete_user_account', {target_user_id})` desde el route handler. | Fundacion / Fase 1 (Plan 01-12 Task 1 anade ADR-009 + fix) | CC | Descubierto 2026-06-07 ejecutando Plan 01-10 Task 1. No bloquea Phase 1 ship (orphan auth rows son no-data, sin login posible — el `public.user` row borrado rompe FK del JWT cookie; user no puede re-loguearse aunque exista auth.users). |
