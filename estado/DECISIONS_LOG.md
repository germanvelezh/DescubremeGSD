...
- **Pendiente: deploy-smoke** (tras merge) con las 2 cuentas — validar barras Bajo<Medio<Alto (no 100%) + intro por instrumento + circumplejo 4 puntas (no aguja) en el caso dominante. Dos chequeos: (a) etico — "Emociones dificiles"/"Soledad" ALTO rinden las barras mas largas (0.90) en `permacare1`; confirmar que largo+calmo+reencuadre no lee como alarma; (b) copy — intro BFI + `REVEAL_BAND_LEGEND` coexisten en la transicion (posible redundancia, decision de Cowork).
