...
## ADR-033 — Contencion NFR-28 de PERMA en el flujo guiado: interstitial de cuidado safety-only en el choke point `allComplete`, sin construir la superficie de mini-resultado (A2/ADR-031) (2026-07-23) (German + Claude Code)

**Contexto:** el smoke A1 (prod, 2026-07-23; `estado/SMOKE_A1_RESULTADOS_v1.0.md`, hallazgo P1 seguridad) confirmo `[GAP-PERMA-CONTENTION-GUIDED-FLOW]`. La maquinaria NFR-28 esta viva y persistida: el servidor decide la contencion en scoring (`score-session.ts`, umbral del seed PERMA `moderate: PERMA_total < 5.0` Kern), la guarda en `report_snapshot.html_payload.distress` y el assembler/composer la pasan passthrough sin recomputar (`assembler.ts`, `reveal-composer.ts`; T-02-08-02). El flujo guiado SI surfacea contencion — pero solo en el branch `nextCode` de `done/page.tsx` (transicion entre tests → `TransitionScreen` → `ContentionBanner`), que cubre a BFI (pos.1) y TwIVI (pos.3). **Causa raiz: PERMA es el 4o/ultimo** (`db/seeds/product-stack/free/seed.sql`) → cae a `allComplete`, que solo enviaba el email FREE-14 y redirigia. El aterrizaje tampoco lo muestra: el close resuelve solo la sesion de O*NET (`free-close.ts`, `visual_type:"hexagon"`), que no es sensible (`ethics_flags` vacio → `requiresContentionRoute:false`), y `/perfil-integrado` no monta el banner. Snapshot de prod: `severity:"moderate", showContention:true` y el usuario no ve nada. Verificado con Explore contra el codigo actual; los comentarios "dormant — no threshold persisted" en `reporte/page.tsx` estan STALE (el computo se cableo en Plan 02-19).

**Opciones evaluadas (AskUserQuestion, German 2026-07-23), criterio = alcanzabilidad de la senal:**
1. **Interstitial de cuidado en el choke point `allComplete` (ELEGIDA).** Antes de redirigir, si el gate da true, renderizar una pantalla minima (titulo + body + `ContentionBanner` reusado + CTA "Continuar" → close normal). Alcanzabilidad maxima (dispara antes de dispersarse a cualquier terminal, incluido el fallback sin O*NET); cero toque a la composicion del cierre (ADR-031); no construye A2.
2. **Banner en los terminales** (`?cierre=free` + `/perfil-integrado`). Sin pantalla nueva, pero toca la composicion del cierre (zona ADR-031/A2), exige lectura cross-session del distress de PERMA en la sesion de O*NET, y cubrir dos superficies o se escapan usuarios de borde.
3. **Redirigir al reporte propio de PERMA** (`/reporte/{permaSessionId}`, que ya monta el banner). Cero UI nueva, pero esos usuarios se saltan el cierre ADR-031 (captura de nivel obligatoria + ocupaciones Job Zone) — regresion de producto justo para el segmento sensible.

**Decision:** Opcion 1. Un interstitial safety-only en el branch `allComplete`. Separacion explicita del *donde interceptar* (ingenieria: el choke point unico por el que pasa todo PERMA-completer) del *que renderizar* (superficie de producto: la decidio German). El slot que ADR-031 reserva como `mini-resultado PERMA` (antes de la captura de nivel) se ocupa aqui con SOLO la senal de cuidado; cuando A2/OLA 3 construya el mini-resultado real en ese slot, la contencion se pliega dentro (el composer ya emite `showContention`). No se construye A2 por la puerta de atras.

**Consecuencias:**
- **Reuso, no invencion.** El read (`resolveLastScoredSessionId` → `composeReport` → `report.distress` + `report.footer.requiresContentionRoute` → `getContentionResources`) es el mismo del branch `nextCode`; se movio a un helper (`lib/free/guided-contention.ts`) para no duplicarlo en la pagina y para no tocar el branch `nextCode` que ya funciona. El componente `ContentionBanner` se reusa intacto (firmado, "no rehacer"): el patron dual (banner prominente + footer discreto) es identico al que el reporte ya despacha.
- **Gate como funcion pura testeada** (`lib/free/contention-gate.ts`, `shouldSurfaceContention`): `requiresContentionRoute && showContention`. 7 unit tests (caso core PERMA-bajo, sin-distress no surfacea, guard no-sensible, fail-safe null/undefined) — mismo patron que `runner-navigation.ts`. Ademas un render test de la SUPERFICIE (`tests/integration/perma-care-screen.test.tsx`): banner `role="complementary"` + lineas `tel:` + CTA cuyo `href` = el close, mismo estandar que el hermano `transition-screen-mini-result.test.tsx`. Fail-safe: input faltante/degradado → no surfacea (un estado desconocido no es distress; no se fabrica una pantalla de cuidado). Lineas vacias → tampoco surfacea (un banner sin ruta no ofrece nada) → redirige normal.
- **Mismo destino de cierre para quien el servidor NO marco.** `showContention:false` → redirige al mismo close de siempre, sin pantalla nueva (corre un `composeReport` extra para leer el gate: mismo destino, no misma ejecucion). Tests 1-3 intactos (su contencion via `nextCode` no se toca). FREE-14 email + resolucion del close preservados (el email sigue await-eado antes del render; el `closeId` alimenta el CTA "Continuar").
- **Copy nuevo firmado por Cowork (esta sesion):** `MC_PERMA_CARE_SCREEN_{HEADING,BODY,CTA}` en `lib/i18n/microcopy/es-CO/nfr28.ts` (hogar correcto: toda la contencion en un modulo auditable). Body **sin repetir** "gratuitas y confidenciales" (ya en `MC_NFR28_BANNER_BODY`, misma vista — verificado contra el componente). aria del landmark reusa `MC_REPORT_CONTENTION_LANDMARK_ARIA`. NFR-27 intacto.
- **Etica:** pantalla calma, papel estatico, sin motion de clima/noche (el delight nunca se apoya en una senal de bienestar bajo; mismo principio que el `tone==='sensitive'` de `TransitionScreen`). Ofrece la ruta, no obliga ("puedes seguir sin contactar a nadie").
- **Alcance del gate (honestidad):** gatea sobre el ULTIMO test completado (`instrumentCode`). El flujo guiado completa PERMA ultimo por el orden sembrado, y PERMA es hoy el unico instrumento con umbral de distress activo (BFI: `showContention:false`, triggers item-level dormant), asi que el ultimo-completado ES el que puede portar la senal. Si un futuro flujo NO guiado pudiera completar PERMA no-ultimo, habria que escanear todas las sesiones sensibles completadas. Anotado, no hecho.
- **Degradacion aceptable:** si el scoring/compose de PERMA falla (best-effort), no se muestra la pantalla — misma degradacion (logueada) que hoy tiene el mini-resultado del `nextCode`. La senal sigue alcanzable fuera del flujo (reporte propio de PERMA).

**Reversibilidad:** Alta. Es un branch aditivo gated en el choke point + un componente presentacional + 3 claves de copy; no toca schema, snapshot, scoring, ni la composicion del cierre (ADR-031). Revertir = quitar el `if (care.surface)` y sus 3 archivos nuevos.

**Referencias:**
- Codigo: `lib/free/contention-gate.ts` (+ `.test.ts`), `lib/free/guided-contention.ts`, `app/(b2c)/test/[code]/_components/PermaCareScreen.tsx`, `app/(b2c)/test/[code]/done/page.tsx` (branch `allComplete`), `lib/i18n/microcopy/es-CO/nfr28.ts` (3 claves).
- `estado/SMOKE_A1_RESULTADOS_v1.0.md` (hallazgo P1 seguridad + evidencia prod `severity:"moderate", showContention:true`).
- Reusa: `ContentionBanner` + `getContentionResources` (lineas CO del seed D1.7) + `resolveLastScoredSessionId` + `composeReport`.
- **Separado de** `[GAP-PERMA-MINIRESULT-SURFACE]` (A2, decision de superficie de German, ADR-031, diferido a OLA 3): este ADR cierra SOLO el defecto de seguridad. Relacionado: ADR-031 (cierre canonico Free), ADR-029 (funnel invertido).
- Flag de copy abierto (fuera de alcance, anotado en BACKLOG): `MC_NFR28_BANNER_HEADING` = "...no estas **solo**" (marca de genero masculina, tension con §9); firmado y compartido (BFI/PERMA/VALUES) → item propio, no se pliega aqui.

---

## ADR-034 — Pase visual del reporte Free: ancho de barra por banda (discreto) y radio del circumplejo por escala fija (nunca cero) — extiende ADR-032 (2026-07-23) (Cowork decisiones B + Claude Code implementacion)

**Contexto:** ADR-032 destapo pero NO cerro dos deudas visuales, confirmadas en el deploy-smoke ADR-033 (prod, 2026-07-23; `estado/SMOKE_ADR033_PERMA_CARE_RESULTADOS_v1.0.md`, snapshots BFI `21b0f79c` / TwIVI `96fe99d5`) y elevadas por German sobre dos capturas de las transiciones. (1) **Barras al 100%** (`[GAP-PERMA-BARS-VISUAL-PASS]`, P1): `BarsWithBands` computaba el ancho como `value/(max ?? 5)`, pero `projectBarsDimensions` nunca setea `max` → toda barra corria con `DEFAULT_MAX=5` y cualquier `value ≥ 5` (sumas BFI 6-30, PERMA 0-10) clampeaba a 100%. (2) **Circumplejo en aguja:** el radio se media desde el MRAT (centro = media de la persona), asi que por construccion ~la mitad de los HOV caia ≤0 y colapsaba a muñon; en un perfil de un dominante claro, una aguja. Cowork produjo dos decision docs de spec (no codigo); Claude Code deriva la implementacion.

**Decisiones (Cowork, firmadas 2026-07-23; ambas Opcion B):**
1. **Barras — el largo sigue a la BANDA (discreto), no a la magnitud.** Tres anchos fijos, distintos y monotonos (`BAND_RATIO` = Bajo 0.35 / Medio 0.62 / Alto 0.90). La banda ya es ipsativa (intra-perfil); un ancho por banda hace que barra y banda cuenten la MISMA historia (una magnitud absoluta las contradiria en perfiles homogeneos). `value`/`max` dejan de gobernar el ancho; `max` se elimina del contrato `VisualDimension` (orphan tras el cambio — nunca lo seteaba la proyeccion, ese ERA el bug del clamp). **PERMA lleva marco propio** (intro que nombra el estado del momento + la valencia negativa de Emociones dificiles/Soledad).
2. **Circumplejo — rango relativo, se abandona "centro = media / ≤0 colapsa".** Las 4 direcciones siempre se dibujan con radio real (piso `MIN_TIP_RADIUS=24`, nunca cero); la mas baja recibe el radio mas corto, la mas alta el mayor. Se conserva la oposicion bipolar (orden por `hovAxisOrder`, ADR-032) y se deja de codificar arriba/abajo de la media. Se elimina el aro-base "media=0" (codificaba el concepto abandonado). **Las bandas siguen ipsativas** (se recomputan sobre los HOV centrados para la etiqueta/sr-only); solo cambia el radio.

**Decision del implementador (delegada por el plan §2) — mapeo del radio:** Cowork dio dos constraints en leve tension: "prioridad relativa dentro del rango" y "diferencias pequeñas se vean pequeñas". Un min-max **por perfil** satisface la primera pero VIOLA la segunda (mapea cualquier spread al rango completo → exagera). **Elegido: afin de ESCALA FIJA sobre la media HOV cruda** (escala teorica del instrumento `hovRadiusScale`, TwIVI [1,6], declarada en la familia), normalizada en la proyeccion a `[0,1]`; el componente aplica el piso. Asi: nunca cero, orden preservado, magnitud honesta (spread chico → radios casi iguales), sin normalizacion por perfil. Es reversible; se valida con los 2 perfiles del smoke (dominante → 4 puntas; casi-parejo → no spiky) y, si no cuadra con la intencion de Cowork, se ajusta sin tocar schema.

**Por que la separacion banda/radio es consistente (propiedad load-bearing):** el centrado por MRAT es un shift constante, asi que el orden de la media cruda = orden del centrado = orden de la banda (la banda termina siendo el z-score de las medias crudas). Por eso **el radio mas largo SIEMPRE lleva la banda mas alta**: barra y banda no pueden contradecirse. El radio aporta magnitud honesta; la banda aporta la posicion ipsativa discreta.

**Rutéo del intro BFI-vs-PERMA (FOUND-05-safe):** el intro por instrumento vive como DATO en la familia (`RevealFamily.barsIntroKey`, tipado contra las claves de `report`), NO como condicional en el componente. El assembler resuelve `report[barsIntroKey]` y lo pasa como `VisualProps.intro`; `BarsWithBands` renderiza el texto que reciba → sigue 100% agnostico (cero literal de instrumento en `lib/report` ni en el componente). Alimenta las DOS superficies (transicion `done/page.tsx` + reporte `reporte/page.tsx`), como el `visualDimensions` de ADR-032.

**Copy es-CO firmado por Cowork (esta sesion):** `MC_BARS_LENGTH_NOTE` (regla compartida bajo las barras), `MC_BARS_INTRO_BFI`, `MC_BARS_INTRO_PERMA` (el de PERMA nombra que "Alto" en Emociones dificiles/Soledad = mas de eso, no un defecto), `MC_VALUECIRCLE_NO_ABSENCE_NOTE` (una direccion mas corta pesa menos, no falta). Conservados intactos: `MC_BARS_TABLE_CAPTION`, `MC_REPORT_BAREMO_NOTE`, `MC_VALUECIRCLE_{TITLE,DESC_INTRO,RELATIVE_NOTE}`, `MC_QUALITY_FLAG_NOTE`. No se reuso `MC_REPORT_SCORES_INTRO` (es de O*NET).

**Consecuencias:**
- Cierra `[GAP-PERMA-BARS-VISUAL-PASS]` (P1) y la decision de circumplejo (nueva del brief). Extiende ADR-032: la proyeccion de barras ya no pasa magnitud (ancho por banda) y el circumplejo abandona media=0 (radio por escala fija).
- `rawHovScores` se exporta desde `reveal-composer` como hermana de `centeredHovScores` (una sola cuenta del MRAT, no duplicar — mismo principio que evito el bug original). `centeredHovScores` sigue rigiendo bandas + composer; `rawHovScores` rige el radio.
- **Etica PERMA:** las barras invertidas (Emociones dificiles, Soledad) con color calmo (accent, nunca rojo); el largo no se lee como alarma. La ruta de contencion NFR-28 (ADR-033) sigue aparte (`showContention`), no se acopla a la barra. Circumplejo: nunca radio cero + nota anti-ausencia; un perfil casi-parejo no se dibuja como asimetria fuerte.
- **Tests:** `visual-dimensions.test.ts` reescrito al nuevo contrato (radio = proporcion [0,1] de la media cruda, nunca centrado ni negativo; bandas sobre centrados; QUAL-05 = radios iguales no-cero). `visuals.test.tsx`: regresion de barras (ancho↔banda, no ancho↔value; ninguna al 100%), intro/length-note, y los 5 criterios de radio del §2 (piso nunca-cero, dominante-4-puntas-ganador, casi-parejo-no-spiky, QUAL-05) midiendo el radio real de la punta del poligono. Gates verdes: tsc 0, test:lint 13/13 (FOUND-05 + frases + hardcoded), test:unit 438, `next build` OK.
- **Alcance del guard (honestidad):** el "nunca cero" del radio lo garantiza el PISO del componente (`radiusOf`), no la proyeccion (que puede devolver `value=0` en el piso de escala) — el test lo dice explicito. La escala del radio (px del piso/techo) queda afinable en smoke; los tests asertan invariantes (orden, no-spiky, piso), no px exactos.

**Reversibilidad:** Alta. Ambos cambios son render + copy aditiva + una capa de proyeccion pura; no tocan schema, snapshot ni scoring. Revertir = volver el mapeo de ancho/radio anterior (y re-añadir `max`).

**Referencias:**
- Codigo: `app/(b2c)/reporte/[sessionId]/_components/BarsWithBands.tsx` (`BAND_RATIO`, intro, length-note), `ValueCircle.tsx` (radio con piso, sin branch por signo, sin aro-base, ganador enfatizado, nota anti-ausencia), `_components/visual-registry.ts` (`VisualProps.intro`, `max` removido), `lib/report/visual-dimensions.ts` (`projectCircumplexDimensions` → proporcion normalizada; `unitProportion`), `lib/report/reveal-composer.ts` (`rawHovScores`), `lib/report/assembler.ts` (`visualIntro`), `lib/i18n/microcopy/es-CO/reveal-phrases.ts` (`barsIntroKey`, `hovRadiusScale`), `lib/i18n/microcopy/es-CO/report.ts` (4 claves), `page.tsx` + `done/page.tsx` + `TransitionScreen.tsx` (thread del intro).
- `estado/PLAN_Visuales_Barras_Circumplejo_v1.0.md` (plan ejecutable), `estado/BRIEF_Cowork_Visuales_Barras_Circumplejo_v1.0.md` (brief origen), `estado/SMOKE_ADR033_PERMA_CARE_RESULTADOS_v1.0.md` (evidencia prod).
- Extiende ADR-032 (proyeccion desacoplada), construye sobre la reconstruccion HOV compartida (ADR-032/PR-C). Relacionado: ADR-033 (contencion NFR-28, desacoplada de la barra), `02-UI-SPEC.md` §6.1/§6.2.
- **Alcance del smoke vs la decision del radio (honestidad):** las 2 cuentas del smoke (`permacare1`/`permacontrol2`) son TwIVI OCH-dominante, asi que el smoke solo valida el caso DOMINANTE — donde escala-fija y min-max-por-perfil coinciden. **Divergen solo en el casi-parejo**, que ninguna cuenta sembrada garantiza. Por eso la correccion del casi-parejo descansa en los unit tests (§2.3) + un comparativo estatico generado para la firma de Cowork (dominante + casi-parejo, escala-fija vs min-max: casi-parejo escala-fija spread ~1.8px = casi circulo; min-max spread ~46px = estrella de un perfil casi identico), NO en el smoke. La firma visual del casi-parejo por Cowork queda pendiente antes/junto al merge.
- **Pendiente: deploy-smoke** (tras merge) con las 2 cuentas — validar barras Bajo<Medio<Alto (no 100%) + intro por instrumento + circumplejo 4 puntas (no aguja) en el caso dominante.

**Adenda de firma Cowork (2026-07-24) — las tres consultas del handoff, resueltas:**
- **D1 (radio casi-parejo) — CONFIRMADA la escala fija.** Cowork ratifica: min-max viola la restriccion etica ("un perfil casi identico dibujado como estrella es una mentira visual"); la escala 1-6 es el nivel de respuesta de la propia persona, sigue ipsativo, no reintroduce comparacion normativa; el piso preserva la nota anti-ausencia. **Accion tomada:** fixture durable `NEAR_EQUAL_SCORES` (medias 4.0/4.1/3.9/4.05) en `visual-dimensions.test.ts` — asevera spread de proporciones ~0.04 y que ninguna toca los extremos 0/1 (min-max SIEMPRE forzaria un 0 y un 1) → un refactor a min-max rompe el test. "Hoy lo guarda el ojo una vez; que lo guarde un test siempre."
- **Nota residual (para el acta, sin accion):** el RADIO usa escala fija sobre la media cruda, pero la NARRATIVA del reveal (dominante-vs-par, `composeDominantOrPair`) sigue sobre el HOV CENTRADO (threshold 0.5). Ambas honestas y coherentes; se deja constancia de que visual y narrativa parten de bases distintas por si un caso borde cuenta historias levemente distintas.
